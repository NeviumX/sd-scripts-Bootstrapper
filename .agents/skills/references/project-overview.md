...
- `third_party/custom_scheduler/`: local sparse checkout of the LoRA Easy
  Training Scripts Backend `custom_scheduler`, installed editable into `.venv`.
  It is created and updated by the bootstrapper and is gitignored.
...
The runtime also includes LoRA Easy custom optimizer support through a local
checkout under `third_party/custom_scheduler`. Supporting packages are pinned
with the project dependencies, including `adv-optm==2.2.3`, `bitsandbytes==0.49.2`,
...
present, installs `requirements_pytorch_windows.txt`, installs the local
`third_party/custom_scheduler/custom_scheduler` package editable, runs
`accelerate config default`, then prints versions for key packages including
...
- Keep custom optimizer integration in the project installer, update flow, and
  requirements, not inside the generated `sd-scripts/` checkout.
...
- `third_party/custom_scheduler/`: LoRA Easy Training Scripts Backend の
  `custom_scheduler` を sparse checkout する local directory。`.venv` に editable install される。
  bootstrapper が作成・更新し、gitignore される。
...
runtime には `third_party/custom_scheduler` の local checkout 経由の
LoRA Easy custom optimizer support も含まれます。関連 package として `adv-optm==2.2.3`,
...
`requirements_pytorch_windows.txt` を install し、
`third_party/custom_scheduler/custom_scheduler` package を editable install し、
`accelerate config default` を実行します。最後に
...
- custom optimizer integration は project installer、update flow、requirements 側に置き、
